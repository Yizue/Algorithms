...
class SegmentTree { // the segment tree is stored like a heap array
    private: vi st, A;
        ...
        void build(int p, int L, int R) { // O(n)
            if (L == R) // as L == R, either one is fine
                ...
                // Note: Change the comp operator here if you want max queries
                st[p] = (A[p1] <= A[p2]) ? p1 : p2;  // Min
                // st[p] = (A[p1] >= A[p2]) ? p1 : p2;  // Max
        } }

        int rmq(int p, int L, int R, int i, int j) { // O(log n)
            if (i > R || j < L) return -1; // current segment outside query range
            if (L >= i && R <= j) return st[p]; // inside query range
            // compute the min/max position in the left and right part of the interval
            int p1 = rmq(left(p) , L , (L+R) / 2, i, j);
            int p2 = rmq(right(p), (L+R) / 2 + 1, R , i, j);
            if (p1 == -1) return p2; // if we try to access segment outside query
            if (p2 == -1) return p1; // same as above
            return (A[p1] <= A[p2]) ? p1 : p2; // same as in build routine, Min
            // return (A[p1] >= A[p2]) ? p1 : p2; // Max
        }
        ...
        int update(int p, int L, int R, int idx, int new_value) {
            ...
            // return the position where the overall min/max is
            return st[p] = (A[p1] <= A[p2]) ? p1 : p2; // same as in build routine, Min
            // return (A[p1] >= A[p2]) ? p1 : p2; // Max
        }
    ...
    public:
        SegmentTree(const vi &_A) {
            ...
            st.assign(4 * n, 0); // create large enough vector of 0's
