...
void constructSA_slow() {     // cannot go beyond 1000 characters
    forn (i, n) SA[i] = i; // initial SA: {0, 1, 2, ..., n-1}
...
void countingSort(int k) {   // O(n)
    ...
    forn (i, n)       // count the frequency of each integer rank
        c[i + k < n ? RA[i + k] : 0]++;
    for (i = sum = 0; i < maxi; i++) {
        int t = c[i]; c[i] = sum; sum += t;
    }
    forn (i, n)          // shuffle the suffix array if necessary
        tempSA[c[SA[i]+k < n ? RA[SA[i]+k] : 0]++] = SA[i];
    forn (i, n)                     // update the suffix array SA
        SA[i] = tempSA[i];
}

void constructSA() {   // O (n log n), can go up to ~100K characters
    int i, k, r;
    forn (i, n) RA[i] = T[i];                 // initial rankings
    forn (i, n) SA[i] = i;     // initial SA: {0, 1, 2, ..., n-1}
    for (k = 1; k < n; k <<= 1) {       // repeat sorting process log n times
        countingSort(k);  // actually radix sort: sort based on the second item
        countingSort(0);          // then (stable) sort based on the first item
        tempRA[SA[0]] = r = 0;             // re-ranking; start from rank r = 0
        fornm (i, 1, n)                    // compare adjacent suffixes
            tempRA[SA[i]] = // if same pair => same rank r; otherwise, increase r
            (RA[SA[i]] == RA[SA[i-1]] && RA[SA[i]+k] == RA[SA[i-1]+k]) ? r : ++r;
        forn (i, n)                     // update the rank array RA
            RA[i] = tempRA[i];
...
void computeLCP_slow() {
    ...
    fornm (i, 1, n) {                // compute LCP by definition
        int L = 0;                                // always reset L to 0
        while (T[SA[i] + L] == T[SA[i-1] + L]) L++;    // same L-th char, L++
            LCP[i] = L;
} }

void computeLCP() { // O(n), Lowest Common Prefix
    int i, L;
    Phi[SA[0]] = -1;                         // default value
    fornm (i, 1, n)                          // compute Phi in O(n)
...
ii LRS() { // Finds Longest Repeated Substring
    int i, idx = 0, maxLCP = -1;
    fornm (i, 1, n)                         // O(n), start from i = 1
        if (LCP[i] > maxLCP)
        maxLCP = LCP[i], idx = i;
    return ii(maxLCP, idx); // returns a pair (the LRS length and its index)
}

int owner(int idx) { return (idx < n-m-1) ? 1 : 2; }

ii LCS() { // Finds Longest Common Substring
    int i, idx = 0, maxLCP = -1;
    fornm (i, 1, n)                           // O(n), start from i = 1
        if (owner(SA[i]) != owner(SA[i-1]) && LCP[i] > maxLCP)
            maxLCP = LCP[i], idx = i;
    return ii(maxLCP, idx); // returns a pair (the LCS length and its index)
}
